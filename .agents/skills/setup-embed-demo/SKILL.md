...
> [!CRITICAL]
> **1. NEVER write ad-hoc scratch local Python scripts** to perform these tasks. Rely exclusively on the modular scripts in `./scripts/` and the `lkr-dev-cli` CLI turnkey commands.
...
- Confirm that Looker API credentials are configured via standard Looker SDK environment variables or that the active OAuth account is known via `uvx lkr-dev-cli auth list`.
...
3. **Step 3: LookML Push & Production Deployment**: In the terminal, run the state-of-the-art `lkr-dev-cli` CLI command to push all local LookML files and deploy them to production:
   ```bash
   uvx lkr-dev-cli --oauth-account=<oauth_account_name> tools lookml push lookml --project=<looker_project_name> --deploy
   ```
   *(Match `oauth_account_name` from `uvx lkr-dev-cli auth list` using the target instance URL.)*
4. **Step 4: Model Registration**: Execute `create_lookml_model` via an inline `run_python_code` call (`dev_mode=True`) to link the model name to the target project and database connections:
   ```python
   import os
