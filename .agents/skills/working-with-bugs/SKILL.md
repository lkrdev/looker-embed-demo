---
name: working-with-bugs
description: Strict workflow and interactive loop guidelines for searching, diagnosing, implementing, and closing bug tracking system subtasks in partnership with the user.
---

# Working with Bugs in the Loop

This skill enforces a collaborative, in-the-loop workflow whenever the developer is tasked with resolving a batch of bugs or subtasks from the issue tracker. It prioritizes communication, explicit approvals, and preventing premature actions.

---

## The Workflow Loop

Follow these steps sequentially for every batch of issue tracker tasks:

### 1. Fetch and Filter
* **Search**: Retrieve all subtasks under the requested parent task ID or search query (set `maxResults` to a high number, e.g. 50, to avoid missing items).
...
* **Render**: Call the issue rendering/view tool on the bug ID to read the full description and comments. Do not make assumptions about the requirements without reading the actual issue.
...
* Once approved by the user:
  ...
  4. Call the issue status update tool to mark the issue as `FIXED`.
