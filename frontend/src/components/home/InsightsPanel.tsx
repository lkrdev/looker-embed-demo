      ...
      <Card variant="glass" className="insights-panel flex-col gap-4 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, var(--primary-light), var(--accent-light))', border: '1px solid var(--border)', height: '28rem', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="relative z-10 pt-1 w-full flex-1" style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            ...
            <>
              {canScrollLeft && (
                <>
                  <button
                    type="button"
                    className="insights-floating-arrow left animate-in fade-in duration-200"
              ...
              {canScrollRight && (
                <>
                  <button
                    type="button"
                    className="insights-floating-arrow right animate-in fade-in duration-200"
          ...
          <div
            ...
            className="insights-scroll-track w-full flex-1"
            style={{ minHeight: 0, alignItems: 'stretch' }}
          >
            {isLoading ? (
              <>
                {[1, 2, 3].map((i) => (
                  <Card key={i} variant="default" className="insight-snap-card flex-col gap-3 p-4 bg-surface/90 backdrop-blur-md animate-pulse" style={{ backgroundColor: 'var(--surface)', padding: '16px', borderRadius: '12px', border: '1px solid var(--border)', height: '100%' }}>
              ...
              insights.map((item, idx) => {
                ...
                return (
                  <Card key={item.id} variant="default" className="insight-snap-card flex-col gap-3 p-4 bg-surface/90 hover-lift backdrop-blur-md" style={{ backgroundColor: 'var(--surface)', padding: '16px', borderRadius: '12px', border: '1px solid var(--border)', animationDelay: `${idx * 80}ms`, height: '100%', display: 'flex', flexDirection: 'column' }}>
          ...
          {!isLoading && !error && insights.length > visibleCardsCount && (
            <div className="insights-dot-pills mt-2 animate-in fade-in duration-200" style={{ flexShrink: 0 }}>
              {Array.from({ length: Math.max(1, insights.length - visibleCardsCount + 1) }).map((_, i) => (
                <button
                  key={i}
                  type="button"
                  className={`insights-dot-pill ${i === activeCardIndex ? 'active' : ''}`}
